...
def get_high_scores(self):
    ...
    f.close()
    if len(high_scores) > 6: high_scores = high_scores[:6]
    while len(high_scores) < 6: high_scores.append((0, ""))
    return high_scores
...
def update_high_scores(self):
    ...
    for line in high_scores:
        if self.score >= line[0]:
            high_scores.insert(high_scores.index(line), (self.score, self.player_name()))
            high_scores.pop(-1)
            break
    self.new_score(high_scores)

def create_score_list():
    f = open('high_score.txt', 'r')
    high_score_font = pygame.font.SysFont('quinquefive0wonv', 28)
    high_score_list_font = pygame.font.SysFont('quinquefive0wonv', 15)
    HS_WIDTH =  WIDTH - 300
    HS_HEIGHT = HEIGHT - 400
...
this looks awesome! The only thing left that I want to do is to implement a high score function. 

I want it to:

1) keep track of all scores -- including the score and a three character handle for the player
2) print the top three scores to the screen
3) let the player know when they have earned one of the top three scores
4) ask the player to input their three character handle

When the game ends, I want it to: 
1) PAUSE the screen
2) Play the losing tone
3) keep playing music
4) Determine if the player has earned one of the top three scores
5) Ask the player for their handle
6) If they earned one of the top three scores -- congratulate the player and update the top three scores displayed on screen
7) 
